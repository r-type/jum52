...
//
//  Cross-platform codebase, version 0.70
//
//  Copyright James Higgs 1999-2013
//
//====================================================================
//  5200 emu main prog
//====================================================================
...
uint16 snd_buf_size;                // different for NTSC or PAL, and different sample rates
uint8 *snd = NULL;
int8 *voiceBuffer = NULL;
uint8 currVoiceVal[4] = { 0, 0, 0, 0 };			// value carried over from previous frame
int numSampleEvents[4];
SampleEvent_t sampleEvent[4][MAX_SAMPLE_EVENTS];

uint8 kbcode, kbcode2;					// joy 1 and 2 keyboard
int pot_max_left, pot_max_right;

uint8 *memory5200;						// pointer to 6502 memory
static uint8 *mapper;					// address to peripheral mapping

uint8 trig[4];

Map16k_t *p16kMaps;						// pointer to 16k rom mappings
int num16kMappings;						// number of 16k rom mappings

unsigned short keyMap[KEY_MAP_SIZE];	// for keyboard remapping (includes "extended" keycodes up to MENU key)

const char *emu_version = "Jum's 5200 Emulator V1.1";
