...
// Copyright James Higgs 2004 - 2016
#include <stdlib.h>
#include <io.h>
#include <stdio.h>
#include "windows.h"
...
int Init(void)
{
	...
	pBuffer = SDL_CreateRGBSurface(SDL_SWSURFACE, outScreenWidth, (int)(outScreenHeight * 1.5), 8,	// w, h, depth
																 0, 0, 0, 0);	// rgba masks
	...
	if (memory5200[DMACTL] & 0x10)
	{
		...
		for (i = 0; i < 256; i++)
		{
			for (n = 0; n < 5; n++)
			{
				...
				if (data & 0x80)
					SDL_SetPixel(pBuffer, x, i, 15);
				if (data & 0x40)
					SDL_SetPixel(pBuffer, x + 1, i, 15);
				if (data & 0x20)
					SDL_SetPixel(pBuffer, x + 2, i, 15);
				if (data & 0x10)
					SDL_SetPixel(pBuffer, x + 3, i, 15);
				if (data & 0x08)
					SDL_SetPixel(pBuffer, x + 4, i, 15);
				if (data & 0x04)
					SDL_SetPixel(pBuffer, x + 5, i, 15);
				if (data & 0x02)
					SDL_SetPixel(pBuffer, x + 6, i, 15);
				if (data & 0x01)
					SDL_SetPixel(pBuffer, x + 7, i, 15);
			}
		}
	}
	else
	...
	for (i = 0; i < 16; i++)
	{
		for (j = 0; j < 16; j++)
		{
			for (k = 0; k < 8; k++)
			{
				...
				y = i * 8 + k;
				if (d & 0x80)
					SDL_SetPixel(pBuffer, x, y, c);
				if (d & 0x40)
					SDL_SetPixel(pBuffer, x + 1, y, c);
				if (d & 0x20)
					SDL_SetPixel(pBuffer, x + 2, y, c);
				if (d & 0x10)
					SDL_SetPixel(pBuffer, x + 3, y, c);
				if (d & 0x08)
					SDL_SetPixel(pBuffer, x + 4, y, c);
				if (d & 0x04)
					SDL_SetPixel(pBuffer, x + 5, y, c);
				if (d & 0x02)
					SDL_SetPixel(pBuffer, x + 6, y, c);
				if (d & 0x01)
					SDL_SetPixel(pBuffer, x + 7, y, c);
			}
		}
	}
	BlitBuffer(0, 240);
