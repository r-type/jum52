...
static const a5200hwtable hwtable[150] = {
	// zero page locations
...
static void show_opcode(char *mystring, uint8 instr);
static void show_operand(char *mystring, uint8 instr);


static const char *check_for_hwaddr(uint16 thisaddr) {
	int i;
...
static void add_hw_tag(char *s, uint16 addr) {
	const char *hwname = check_for_hwaddr(addr);
	if (hwname) {
		int len = strlen(s);
