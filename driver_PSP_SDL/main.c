// PSP / SDL Driver for Jum52 (v 1.1)
...
#ifdef PSP
// 320x204 to 480x272 "stretch map address array"
uint8 *stretchMap[480*272];
#endif

int fps;
int fps_on = 0;				// show fps?
int lastSecondFrame = 0;
int lastSecondTick = 0;
int skip = 1;				// frame skip?
int exit_app;
int gotcartlist = 0;
int debugging = 0;
...
*/
	...
	if(options.audio)
		...
		// Convert to SDL sound buffer format
		for (i = 0; i < len; i++)
			{
			// NB: SDL sound buffer is in signed 16-bit format
			short value = audioBuffer[i] - 128;
			value *= vol256;
...
int Init(void)
{
...
#ifdef PSP
	// Only 300MHz and 333 MHz seem to be fast enough to run at 60fps!
	// (Only 333MHz is fast enough for stretched mode)
	...
	//create a window
	screenFlags = SDL_ANYFORMAT;
	//pSurface = SDL_SetVideoMode(SCREENWIDTH, SCREENHEIGHT, SCREENBPP, screenFlags);
	pSurface = SDL_SetVideoMode(SCREENWIDTH, SCREENHEIGHT, 32, SDL_HWSURFACE);
	if( pSurface == NULL )
		{
...
//	SDL_GetWMInfo(&wmInfo);
	...
	// make buffer big enuf for 2x mode
	pBuffer = SDL_CreateRGBSurface(SDL_SWSURFACE,
									SCREENWIDTH, SCREENHEIGHT * 2, SCREENBPP,	// w, h, depth
									0, 0, 0, 0);	// rgba masks
...
#ifdef PSP
	// set up stretch map 320x204 to 480x272
	// NB: vid pointer in 5200emu must have been set up in Jum52_Initialise()
	int y;
	int x;
	int offset = 0;
	for (y = 0; y < 272; y++)
		{
		int v = (int)((float)y * 0.75);
		for (x = 0; x < 480; x++)
			{
			int u = (int)((float)x * 0.6666667);
			// We skip top 24 lines, take 204 lines only
			stretchMap[offset++] = vid + ((v+24) * VID_WIDTH) + u;
			}
		}
#endif
...
#ifndef PSP
	SDL_WM_SetCaption("Jum52 V1.1 PSP/SDL", NULL);
	...
	// set up audio output
	// NB: For PSP, freq must be 44100, format = AUDIO_S16
	reqSpec.freq = 44100;
	...
	if (!audioBuffer)
		{
		printf("Couldn't allocate audioBuffer!\n");
...
void BlitBuffer(int destX, int destY)
{
	...
	SDL_UpdateRect(pSurface, 0, 0, SCREENWIDTH, SCREENHEIGHT);
...
//#define WIDE_MODE 0				// 1.0 x
#define WIDE_MODE 1				// 1.5 x
...
void HostBlitVideo(void) {
	...
	// use frameskip if specified
	if (skip && (framesdrawn & 1))
		return;
	...
	if (0 == WIDE_MODE)
		...
		for(y=0; y<240; y++)
			...
			pScr += (pBuffer->w - 320);	// go to start of next line of SDL buffer
			}
		}
	else if (1 == WIDE_MODE)
		{
		// 1.5x horizontal stretch mode
		pScr = (uint8 *)pBuffer->pixels;
		for(y=0; y<272; y++)
			{
			int v = (int)((float)y * 0.75);
			pLine = vid + ((v+24) * VID_WIDTH);  // skip top 24 lines
			for(x=0; x<160; x++)
				{
				*pScr++ = *pLine++;
				*pScr++ = *pLine++;
			...
			pScr += (pBuffer->w - 480);	// go to start of next line of SDL buffer
			}
/*
		// "Full-screen" mapped stretch mode
		// (1.5x horizontal stretch, 1.333x vertical stretch)
		pScr = (uint8 *)pBuffer->pixels;
		uint8 **p = stretchMap;					// tricky pointer stuffs here
		for(y = 0; y < 272; y++)
			{
			for(x=0; x < 480; x++)
				{
...
*/
	...
	printXY(s, 160, 0, 0x0F);

	for(x=0; x<320; x++) 
		SDL_SetPixel ( pBuffer, x, audioBuffer[x*2]>>1, 0x0F); 
#endif
...
//	}
	...
	if (0 == WIDE_MODE)
		...
		destRect.x = 0;
		destRect.y = 0; //16;
		destRect.w = 480; destRect.h = 272;	// ignored
		}
...
/// eg: PSP
/// @param button		Controller button index
/// @param down			Whether button is pressed (1) or released (0)
/// @return 			SDL key value
SDLKey ControllerButtonToKeyPress(Uint8 button, int pressed)
{
	static int keyShift = 0;
	SDLKey keycode = 0;
	...
	switch (button)
		...
		case 0 : // TRI
			if (keyShift)
				...
				keycode = SDLK_0;		// "0" key
			break;
		case 1 : // CIRCLE
			if (keyShift)
				keycode = SDLK_8;
			else
				keycode = SDLK_RSHIFT;		// Top fire buttons (L & R)
			break;
		case 2 : // X
			if (keyShift)
				keycode = SDLK_9;
			else
				keycode = SDLK_RCTRL;		// Bottom fire buttons (L & R)
			break;
		case 3 : // SQUARE
			keycode = 0;			// use as "shift" in conjunction with DPAD, for number pad
			if (pressed)
				keyShift = 1;
			else
				keyShift = 0;
		...
		case 4 : // LSB
			if (keyShift)
				...
				keycode = SDLK_MINUS;	// "star" key
			break;
		case 5 : // RSB
			if (keyShift)
				keycode = SDLK_6;
			else
				keycode = SDLK_EQUALS;	// "hash" key
			break;
		case 6 : // DOWN
			if (keyShift)
				keycode = SDLK_3;
		...
		case 9 : // RIGHT
			...
			break;
		case 10 : // SELECT
			keycode = SDLK_ESCAPE;		// Escape to menu
			break;
		case 11 : // START		
			keycode = SDLK_F1;			// "start"				
			break;
		case 12 : // HOME		
			keycode = 0;				
			break;
		case 13 : // HOLD		
			keycode = SDLK_F2;			// "pause"				
			break;
		}
...
void HostDoEvents(void) {
	...
	{
#ifdef PSP		
		// Translate controller button presses to keyboard input
		if (SDL_JOYBUTTONDOWN == event.type)
			{
			event.type = SDL_KEYDOWN;
			event.key.keysym.sym = ControllerButtonToKeyPress(event.jbutton.button, 1);
			}
		else if (SDL_JOYBUTTONUP == event.type)
			{
			event.type = SDL_KEYUP;
			event.key.keysym.sym = ControllerButtonToKeyPress(event.jbutton.button, 0);
			}
#endif			
		switch( event.type ) {
			case SDL_QUIT :
			...
			case SDL_KEYDOWN:
				...
				switch(event.key.keysym.sym) {
					...
					case SDLK_KP_MINUS :	// Reset button PL2
						cont2.key[4] = 1;
						break;
					case SDLK_ESCAPE :		// Escape to options menu
						running = 0;
						break;
...
//	struct _finddata_t fileinfo;
	...
	while(1)
		...
		if( SDL_PollEvent ( &event ) )
			...
			switch( event.type )
				{
				case SDL_JOYBUTTONDOWN:
					key = ControllerButtonToKeyPress(event.jbutton.button, 0);
					break;
				case SDL_KEYDOWN:
		...
		if(key == SDLK_SPACE || key == SDLK_RETURN || key == SDLK_RCTRL)
			break;
		...
		if(key == SDLK_PAGEDOWN)
			{
			frame_position += 10;
			if(frame_position > (numroms-1))
				frame_position = (numroms-1);
			//debounce = DEBOUNCE_RATE;
			}
		...
		printXY("Select Game ROM", 60, 8, 0x4E);
		...
		for(i=frame_position;i<(frame_position+19);i++)
			...
			if(i>=0 && i < numroms)
				{
				if(i == selection)
					printXY(romdata[i].name, 60, y, 0x0f);
				else
					printXY(romdata[i].name, 60, y, 0x18);
				}
			y += 8;
			}
		...
		SDL_Rect destRect;
		destRect.x = (SCREENWIDTH - 320) / 2;
		destRect.y = (SCREENHEIGHT - 240) / 2;
		destRect.w = 320; destRect.h = 240;		// ignored
		ret = SDL_BlitSurface(pBuffer, NULL, pSurface, &destRect);
		if(ret < 0)
			fprintf(stderr, "BlitSurface error: %s\n", SDL_GetError());
	...
	rect.w = SCREENWIDTH;
	rect.h = SCREENHEIGHT;
	SDL_FillRect(pSurface, &rect, SDL_MapRGB(pSurface->format, 32, 32, 32));
	SDL_UpdateRect(pSurface, 0, 0, SCREENWIDTH, SCREENHEIGHT);
...
void DoHelpMenu()
{
	...
	destRect.y = (SCREENHEIGHT - 240) / 2;
	destRect.w = 320; destRect.h = 240;		// ignored
	int ret = SDL_BlitSurface(pBuffer, NULL, pSurface, &destRect);
	if(ret < 0)
		fprintf(stderr, "BlitSurface error: %s\n", SDL_GetError());
...
int DoOptionsMenu(void)
{
	...
	while(!exit_menu)
		...
		if(key == SDLK_SPACE || key == SDLK_RETURN || key == SDLK_RCTRL)  // SPACE/RETURN/FIRE
			{
			if(selected_row == 0)
				{
				DoHelpMenu();
		...
		// return to game?
		if(key == SDLK_ESCAPE || key == SDLK_F6)	// esc or TRIANGLE
			{
			selected_row = -1;
			Wait(30);
			break;
		...
		if((key == SDLK_RIGHT) || (key == SDLK_LEFT))
			...
			switch(selected_row)
				...
				case 4 : // controller
					options.controller++;
#ifdef PSP
					if(2 == options.controller) options.controller = 0;
#else
		...
		for(i=0;i<=SELECT_ROW_MAX; i++)
			...
			switch(i)
				...
				case 4:
#ifdef PSP
					strcpy(s, "Controller: DPAD");
					if(1 == options.controller) strcpy(s, "Controller: Analog Nub");
#else
...
HostLog(text);
	...
	printf("Cleaning up rom list array...\n");
	if (romdata)	
		free(romdata);
